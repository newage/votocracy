...
{
    private const PROMETHEUS_NAME = '%s_%s_%s_%s{environment="%s", uid="%s", response="%s"} %d %s';

    public function __construct(private readonly Runner $runner, private readonly ConfigInterface $config)
    {
    }
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $strings[] = sprintf(
            '%s_http_life{environment="%s", uid="%s", response="Ok"} 1 0',
