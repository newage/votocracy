...
class LoggerMiddlewareFactory
{
    ...
    {
        $callable = function ($requestId, bool $debugEnableHeader) use ($container) {
            ...
            if (($className = $config->get('logging.writer'))) {
                $parameters = array_values($config->get('logging.parameters', []));
                $writer = new $className(...$parameters);
            } else {
            ...
            /* log all levels more or equal than NOTICE. Without INFO,DEBUG level */
